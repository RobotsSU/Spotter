...
motocmd = 0
while True:
    connection, address = phonesock.recvfrom(8192)
    while True:
        if True:
            ...
            for axis_no in xrange(0, len(axes), 2):
                ...
                text= font.render("%s   %s   %s   %s  %s   %s" % (address,motocmd,axis_x,axis_y,serva,servb),1, (10,10,10))
                if axis_no == 0:
		    ...
		    #if  axis_y == 0:
                    #    motocmd = 128
                    servb = "\xff\x01%c" %(int(axis_y * -28) + 75)
                if axis_no == 1:
                    #phonesock.sendto("\xff\x02",(phoneaddr,phoneport))
                    servc = "\xff\x03%c" % (int(axis_x * -28) + 75)
                ...
                x += axis_size
            sleep(.05)
            phonesock.sendto(serva, address)
            phonesock.sendto(servb, address)
            phonesock.sendto(servc, address)
